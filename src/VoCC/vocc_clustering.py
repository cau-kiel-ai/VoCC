...
class VortexCorrelationClustering(ClusterMixin, BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        assert X.shape[1] == 4, 'The input array must be structured as (n_samples, 4) for the propoerties position X, position Y, movement X, movement Y along the second dimension'

        X, Y, U, V = X.T
        normal_vectors = _bin_image(X, Y, U, V, self.cell_size)

        accumulator = _create_parameter_space(normal_vectors, self.radii, self.sectors, self.circle_coverage_rate)
        ...
        particle_tree = KDTree(np.array([X,Y]).T)
...
class Vortex():
    def __init__(self, x_bin: int, y_bin: int, radius: int, pixels: np.ndarray, flag):
        self.x_bin = x_bin 
        self.y_bin = y_bin
        self.radius = radius 
        self.pixels = pixels
...
def _bin_image(X, Y, U, V, cell_size):
    ...
    movement = np.zeros(shape = (bins_X.shape[0] +1, bins_Y.shape[0]+1, 2))
    normal = np.zeros(shape = (bins_X.shape[0] +1, bins_Y.shape[0]+1, 2))
    
    movement[X_indices, Y_indices, 0] = (U / cell_size).squeeze()
    movement[X_indices, Y_indices, 1] = (V / cell_size).squeeze()
...
def _create_parameter_space(normal_vectors, radii, sectors, circle_coverage_rate):
    accumulator = []
    for radius in radii:
        XX, YY = np.meshgrid(np.arange(2 * radius +1), np.arange(2* radius +1 ))

        distances_from_center =(XX-radius)**2 + (YY-radius)**2
        activation_mask = (distances_from_center < radius ** 2).astype(np.float)
        ...
        for sector_index in range(sectors):
            ...
            sector_vector_i = convolve(normal_vectors[..., 00], ht_mask_x) + convolve(normal_vectors[..., 1], ht_mask_y)

            sector_accumulator.append(sector_vector_i)
        
        # qth-quantile selection of the sector vectors
        sector_accumulator = np.array(sector_accumulator).reshape(sectors, normal_vectors.shape[0], normal_vectors.shape[1])
...
def _extract_vortices_from_parameter_space(accumulator, threshold, radii, flag):
    ...
    accumulator_high_values = np.maximum(accumulator, np.full_like(accumulator, fill_value=threshold))
    ...
    for index in indices.T:
        vortices.append(Vortex(index[0],index[1], radii[accumulator2d_indices[index[0], index[1]]], [], flag))
...
def _merge_votex_candidates(vortices, radii, rdbscan_min_points):
    ...
    X = np.empty(shape = (len(vortices), 3))
    for vortex_i, vortex in enumerate(vortices):
        X[vortex_i] = [vortex.x_bin, vortex.y_bin, vortex.radius]
    ...
    for l in np.unique(labels):
        ...
        if np.sum(mask) == 1:
            vortices_grouped.append(vortices[np.argwhere(mask).squeeze()])
        elif np.sum(mask) > 1:
            indices = np.argwhere(labels == l).squeeze()

            pixels = []
            inner_x_y_r = np.empty(shape= (len(indices), 3))
            for index_i, index in enumerate(indices):
                vortex = vortices[index]
                inner_x_y_r[index_i] = [vortex.x_bin, vortex.y_bin, vortex.radius]
            ...
            vortex_group = Vortex(new_x_bin, new_y_bin, np.max(inner_x_y_r[:, 2]), inner_x_y_r, vortex.flag)
...
def _radius_eps_DBSCAN(X: np.ndarray, min_samples : int):
    """
    X is an array (n, 3) with x, y, and radius.
    """
    assert X.shape[1] == 3, "Please use three dimensions for the radius DBSCAN"

    tree = KDTree(X[:, :2])
    neighborhoods = np.empty(shape = (X.shape[0]), dtype = object)

    for r in np.unique(X[:, 2])[::-1]:
        r_mask = X[:, 2] == r
        neighbors_per_r = tree.query_radius(X[r_mask, :2],r)
        ...
        neighborhoods[r_mask] = neighbors_per_r
